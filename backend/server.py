...
async def list_supplier_products(
    ...
    links = await db.supplier_products.find(query, {"_id": 0}).to_list(5000)
    if not links:
        return []

    product_ids = list({link.get("product_id") for link in links if link.get("product_id")})
    if not product_ids:
        return []

    product_query: Dict[str, Any] = {"user_id": owner_id, "product_id": {"$in": product_ids}}
    product_query = apply_accessible_store_scope(product_query, user)
    scoped_products = await db.products.find(product_query, {"_id": 0, "product_id": 1}).to_list(5000)
    allowed_product_ids = {item.get("product_id") for item in scoped_products if item.get("product_id")}
    if not allowed_product_ids:
        return []

    return [link for link in links if link.get("product_id") in allowed_product_ids]
...
async def update_supplier_product(link_id: str, link_data: SupplierProductUpdate, user: User = Depends(require_permission("suppliers", "write"))):
    owner_id = get_owner_id(user)
    existing = await db.supplier_products.find_one({"link_id": link_id, "user_id": owner_id}, {"_id": 0})
    if not existing:
        raise HTTPException(status_code=404, detail="Lien non trouve")

    supplier = await db.suppliers.find_one({"supplier_id": existing.get("supplier_id"), "user_id": owner_id}, {"_id": 0})
    supplier = await backfill_legacy_store_field(
        db.suppliers,
        {"supplier_id": existing.get("supplier_id"), "user_id": owner_id},
        supplier,
        user,
    )
    if not supplier:
    ...
    update_fields = {k: v for k, v in link_data.model_dump().items() if v is not None}
    if update_fields:
        await db.supplier_products.update_one(
            {"link_id": link_id, "user_id": owner_id},
            {"$set": update_fields}
        )
    ...
    updated = await db.supplier_products.find_one({"link_id": link_id, "user_id": owner_id}, {"_id": 0})
    return SupplierProduct(**updated)
...
async def create_order(order_data: OrderCreate, user: User = Depends(require_procurement_access("write"))):
    owner_id = get_owner_id(user)
    resolved_supplier_id = order_data.supplier_id
    ...
    if is_connected:
        ...
        if not profile:
            raise HTTPException(status_code=404, detail="Fournisseur marketplace non trouve")
        existing_local_supplier = await db.suppliers.find_one(
            {"user_id": owner_id, "linked_user_id": order_data.supplier_user_id},
            {"_id": 0},
        )
        if not existing_local_supplier:
            local_supplier = Supplier(
                ...
                notes="Ajoute automatiquement depuis le marketplace",
                products_supplied=", ".join([c for c in (profile.get("categories") or []) if c]),
                delivery_delay=f"{profile.get('average_delivery_days', 0)} jours" if profile.get("average_delivery_days") else "",
            )
            local_supplier_doc = local_supplier.model_dump()
            local_supplier_doc["linked_user_id"] = order_data.supplier_user_id
            local_supplier_doc["source"] = "marketplace"
            local_supplier_doc["is_connected"] = True
            await db.suppliers.insert_one(local_supplier_doc)
            resolved_supplier_id = local_supplier.supplier_id
        else:
            resolved_supplier_id = existing_local_supplier.get("supplier_id") or order_data.supplier_id
            await db.suppliers.update_one(
                {"supplier_id": resolved_supplier_id, "user_id": owner_id},
                {
                    "$set": {
                        "store_id": existing_local_supplier.get("store_id") or user.active_store_id,
                        "name": profile.get("company_name") or existing_local_supplier.get("name"),
                        "contact_name": profile.get("company_name") or existing_local_supplier.get("contact_name"),
                        "phone": profile.get("phone") or existing_local_supplier.get("phone"),
                        "address": profile.get("address") or existing_local_supplier.get("address"),
                        "updated_at": datetime.now(timezone.utc),
                        "linked_user_id": order_data.supplier_user_id,
                        "source": "marketplace",
                        "is_connected": True,
                    }
                },
...
async def search_marketplace_suppliers(
    ...
    if country_code:
        ...
        supplier_users = await db.users.find(
            {"role": "supplier", "country_code": cc},
            {"_id": 0, "user_id": 1}
        ).to_list(3000)
        supplier_ids = [u.get("user_id") for u in supplier_users if u.get("user_id")]
        if not supplier_ids:
            return []
        filters.append({"user_id": {"$in": supplier_ids}})
    ...
    if q:
        ...
        q_or_filters: List[Dict[str, Any]] = [
        ...
        if catalog_supplier_ids:
            q_or_filters.append({"user_id": {"$in": catalog_supplier_ids}})
        filters.append({"$or": q_or_filters})
    ...
    if category:
        ...
        cat_or_filters: List[Dict[str, Any]] = [
            {"categories": {"$elemMatch": cat_regex}},
        ]
        if category_supplier_ids:
            cat_or_filters.append({"user_id": {"$in": category_supplier_ids}})
        filters.append({"$or": cat_or_filters})

    if city:
        filters.append({"city": {"$regex": safe_regex(city), "$options": "i"}})
    if min_rating is not None:
        filters.append({"rating_average": {"$gte": min_rating}})
    if verified_only:
        filters.append({"is_verified": True})

    query: Dict[str, Any] = {"$and": filters} if filters else {}
