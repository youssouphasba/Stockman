...
function TermsOfService() {
    ...
    useEffect(() => {
        let isMounted = true;
        setLoading(true);
        fetchLegalContent('cgu', i18n.language === 'ar' ? 'ar' : i18n.language === 'en' ? 'en' : 'fr')
            .then((res) => {
                if (isMounted) {
            ...
            .catch(() => {
                if (isMounted) {
                    setContent(t('terms.error_loading', "Impossible de charger les conditions générales d'utilisation."));
                }
            })
            .finally(() => {
        ...
        <div className="landing-page">
            ...
            <section className="container" style={{ padding: 'var(--spacing-xl) 0' }}>
                <div className="glass-card">
                    <h1>{t('terms.title', "Conditions Générales d'Utilisation")}</h1>
                    {updatedAt && (
                        <p className="text-muted" style={{ fontSize: '0.8rem', marginBottom: '20px' }}>
                            {t('terms.last_updated', 'Dernière mise à jour :')} {new Date(updatedAt).toLocaleDateString(i18n.language === 'fr' ? 'fr-FR' : 'en-US')}
                        </p>
                    )}
