    ...
    const handleExportPdf = async () => {
        ...
        try {
            await generateAndSharePdf({
                ...
                kpis: [
                    { label: t('crm.clients'), value: customerList.length.toString() },
                    { label: t('accounting.total_revenue_kpi'), value: formatCurrency(totalSpent, user?.currency) },
                    { label: t('crm.debts'), value: formatCurrency(totalDebt, user?.currency), color: totalDebt > 0 ? '#f44336' : '#4CAF50' },
                ],
                sections: [{
                    title: t('crm.client_list'),
                    headers: [
                        t('crm.col_name'),
                        t('crm.col_phone'),
                        t('crm.col_tier'),
                        t('crm.col_purchases'),
                        t('crm.col_total_revenue'),
                        t('crm.col_debt')
                    ],
                    alignRight: [3, 4, 5],
                    rows: customerList.map((c) => [
                        c.name,
                        c.phone || '-',
                        t(getTierConfig(c.tier).labelKey),
                        (c.visit_count || 0).toString(),
                        formatCurrency(c.total_spent || 0, user?.currency),
                        formatCurrency(c.current_debt || 0, user?.currency)
                    ]),
                }]
            });
        } catch (error) {
