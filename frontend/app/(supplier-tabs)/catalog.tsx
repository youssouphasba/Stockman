...
function getIssues(product: CatalogProductData) {
  return [
    ...
    !product.description?.trim() ? 'Description à compléter' : '',
  ].filter(Boolean);
}
...
export default function SupplierCatalogScreen() {
  ...
  async function duplicateProduct(product: CatalogProductData) {
    try { await supplierCatalog.duplicate(product.catalog_id); await loadProducts(); Alert.alert('Produit dupliqué', 'Une copie a bien été créée.'); }
  ...
  async function updateVisibility(product: CatalogProductData, available: boolean) {
    ...
    catch { Alert.alert('Erreur', 'La mise à jour de la visibilité a échoué.'); }
  }

  async function deleteProduct(product: CatalogProductData) {
    Alert.alert('Supprimer le produit', `Voulez-vous vraiment supprimer "${product.name}" ?`, [{ text: t('common.cancel'), style: 'cancel' }, { text: t('common.delete'), style: 'destructive', onPress: async () => { try { await supplierCatalog.delete(product.catalog_id); await loadProducts(); } catch { Alert.alert('Erreur', 'La suppression a échoué.'); } } }]);
  }
  ...
  function openQuickEdit(source: 'selected' | 'filtered' | 'all' = 'filtered') {
    ...
    if (!sourceProducts.length) return Alert.alert('Aucun produit', "Aucun produit n'est disponible pour l'édition rapide.");
    ...
    setQuickEditSourceLabel(source === 'selected' ? `${sourceProducts.length} produit(s) sélectionné(s)` : source === 'all' ? 'tout le catalogue' : 'la liste visible');
  ...
  async function saveQuickEditRows() {
    ...
    try {
      ...
      setShowQuickEdit(false); await loadProducts(); Alert.alert('Édition rapide', 'Les produits ont été mis à jour.');
  ...
  async function updateSelectedVisibility(available: boolean) {
    ...
    setBulkSaving(true);
    try { await supplierCatalog.bulkUpdate(selectedProducts.map((product) => ({ catalog_id: product.catalog_id, available }))); clearSelection(); setSelectionMode(false); await loadProducts(); Alert.alert('Sélection multiple', available ? 'Les produits sont maintenant visibles.' : 'Les produits sont maintenant masqués.'); }
    catch { Alert.alert('Erreur', 'La mise à jour en lot a échoué.'); } finally { setBulkSaving(false); }
  }

  async function deleteSelectedProducts() {
    if (!selectedProducts.length) return;
    Alert.alert('Supprimer la sélection', `Voulez-vous supprimer ${selectedProducts.length} produit(s) ?`, [{ text: t('common.cancel'), style: 'cancel' }, { text: t('common.delete'), style: 'destructive', onPress: async () => { setBulkSaving(true); try { await supplierCatalog.bulkDelete(selectedProducts.map((product) => product.catalog_id)); clearSelection(); setSelectionMode(false); await loadProducts(); Alert.alert('Sélection multiple', 'Les produits sélectionnés ont été supprimés.'); } catch { Alert.alert('Erreur', 'La suppression en lot a échoué.'); } finally { setBulkSaving(false); } } }]);
  }
  ...
  async function createProductsFromText() {
    ...
    try { await supplierCatalog.bulkCreate(drafts.map((draft) => ({ name: draft.name, description: draft.description, category: draft.category, price: draft.price, stock_available: draft.stock_available, unit: draft.unit || 'pièce', available: true, publication_status: 'published' }))); setTextImportValue(''); setShowTextImport(false); await loadProducts(); Alert.alert('Création par texte', `${drafts.length} produit(s) ont été créés.`); }
    catch { Alert.alert('Erreur', "La création des produits à partir du texte a échoué."); } finally { setTextImportSaving(false); }
  }
  ...
  return (
    <LinearGradient colors={[colors.bgDark, colors.bgMid, colors.bgLight]} style={styles.screen}>
      <ScrollView style={styles.screen} contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); loadProducts(); }} tintColor={colors.secondary} />}>
        ...
        <Text style={styles.subtitle}>Retrouvez les usages essentiels du compte commerçant adaptés au fournisseur : import CSV, création par texte, édition rapide du prix et du stock, puis sélection multiple.</Text>
        ...
        <View style={styles.actionRow}>
          <TouchableOpacity style={[styles.secondaryAction, selectionMode && styles.secondaryActionActive]} onPress={toggleSelectionMode}>
            ...
            <Text style={[styles.secondaryActionText, selectionMode && styles.secondaryActionTextActive]}>{selectionMode ? 'Quitter la sélection' : 'Sélection multiple'}</Text>
          </TouchableOpacity>
          {selectionMode ? <TouchableOpacity style={styles.secondaryAction} onPress={selectAllFiltered}><Ionicons name="checkmark-done-outline" size={16} color={colors.textSecondary} /><Text style={styles.secondaryActionText}>Tout sélectionner</Text></TouchableOpacity> : null}
        </View>
        ...
        {selectionMode ? <View style={styles.infoBanner}><Text style={styles.infoBannerTitle}>{selectedIds.length ? `${selectedIds.length} produit(s) sélectionné(s)` : 'Sélectionnez un ou plusieurs produits'}</Text><Text style={styles.infoBannerText}>Utilisez ensuite la barre d’actions en bas pour modifier les prix, le stock, la visibilité ou supprimer plusieurs produits d’un coup.</Text></View> : null}
            ...
            <TouchableOpacity key={product.catalog_id} activeOpacity={selectionMode ? 0.85 : 1} onPress={() => { if (selectionMode) toggleSelectedProduct(product.catalog_id); }} style={[styles.card, selectionMode && selected && styles.cardSelected]}>
              <View style={styles.cardHeader}>
                <View style={styles.cardHeaderLeft}>
                  ...
                  <View style={[styles.badge, visible ? styles.badgeVisible : styles.badgeHidden]}><Text style={[styles.badgeText, visible ? styles.badgeTextVisible : styles.badgeTextHidden]}>{visible ? 'Visible' : 'Masqué'}</Text></View>
                </View>
              ...
              {issues.length > 0 ? <Text style={styles.warning}>{issues.join(' - ')}</Text> : <Text style={styles.ok}>Fiche complète et exploitable.</Text>}
      ...
      {showActions ? <Modal visible={showActions} animationType="slide" transparent onRequestClose={() => setShowActions(false)}><View style={styles.modalBg}><View style={styles.modal}><Text style={styles.modalTitle}>Actions rapides</Text><Text style={styles.modalSubtitle}>Ce menu regroupe maintenant les vraies actions utiles du catalogue fournisseur.</Text>{[
        { icon: 'cloud-upload-outline', title: 'Importer un CSV', text: 'Importer plusieurs produits à partir d’un fichier et contrôler les colonnes avant validation.', onPress: () => { setShowActions(false); setShowImportModal(true); } },
        { icon: 'document-text-outline', title: 'Création par texte', text: 'Coller plusieurs lignes au format nom ; prix ; stock ; unité ; catégorie ; description.', onPress: () => { setShowActions(false); setShowTextImport(true); } },
        { icon: 'create-outline', title: 'Édition rapide', text: 'Modifier en lot les prix, les stocks et la visibilité sur la liste visible.', onPress: () => openQuickEdit('filtered') },
        { icon: 'checkbox-outline', title: 'Sélection multiple', text: 'Sélectionner plusieurs produits puis agir en lot : édition rapide, visibilité ou suppression.', onPress: toggleSelectionMode },
        { icon: 'add-circle-outline', title: 'Nouveau produit', text: 'Ouvrir la fiche complète comme sur le compte commerçant.', onPress: () => { resetForm(); setShowActions(false); setShowForm(true); } },
...
const createStyles = (colors: any) => StyleSheet.create({
  ...
  quickEditList: { paddingBottom: Spacing.md }, quickCard: { backgroundColor: colors.glass, borderWidth: 1, borderColor: colors.glassBorder, borderRadius: BorderRadius.lg, padding: Spacing.md, marginBottom: Spacing.sm }, quickCardTitle: { color: colors.text, fontWeight: '700', fontSize: FontSize.md, marginBottom: Spacing.xs }, previewCard: { backgroundColor: colors.glass, borderWidth: 1, borderColor: colors.glassBorder, borderRadius: BorderRadius.lg, padding: Spacing.md, marginBottom: Spacing.md }, previewTitle: { color: colors.text, fontWeight: '700', fontSize: FontSize.sm, marginBottom: 6 }, previewText: { color: colors.textSecondary, lineHeight: 20, fontSize: FontSize.sm }, previewError: { color: colors.danger, fontSize: FontSize.sm, lineHeight: 20 },
  helperCard: { backgroundColor: colors.glass, borderWidth: 1, borderColor: colors.glassBorder, borderRadius: BorderRadius.lg, padding: Spacing.md, marginBottom: Spacing.md }, helperTitle: { color: colors.text, fontWeight: '700', fontSize: FontSize.sm, marginBottom: 6 }, helperText: { color: colors.textSecondary, fontSize: FontSize.sm, lineHeight: 20, marginBottom: 4 }, fieldLabel: { color: colors.textSecondary, fontWeight: '700', fontSize: FontSize.xs, marginBottom: 6 },
