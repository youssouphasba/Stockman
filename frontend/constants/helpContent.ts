...
export const HELP_MODULES: HelpModule[] = [
  ...
  },
  {
    ...
    features: [
      ...
      { icon: 'storefront-outline', title: "Site e-commerce", description: "Stockman prépare un site public désactivé par défaut. L'administrateur peut l'activer, choisir la boutique liée, personnaliser le nom du site, l'accueil, les contacts, la livraison, WhatsApp, le paiement manuel et configurer le domaine. Le format recommandé pour un domaine connecté est www.votredomaine.com : créez un CNAME vers Stockman, puis redirigez votredomaine.com vers ce sous-domaine. Les réglages affichent maintenant un guide concret selon Cloudflare, OVHcloud ou GoDaddy, avec le type, le nom et la valeur DNS à saisir. Les couleurs se choisissent avec des pastilles visuelles et pilotent les boutons, badges et accents du site. La vitrine affiche une identité de boutique visible dès l'en-tête, une vraie section Contactez-nous et un formulaire public qui envoie la demande au commerçant. Le bouton E-com permet de voir le site, d'ouvrir les statistiques et d'accéder aux paramètres. Les commandes E-com arrivent dans Stockman avec notifications push, in-app et e-mail. Le client reçoit aussi un e-mail de confirmation puis des mises à jour de statut si une adresse e-mail a été renseignée. Depuis le détail d'une commande E-com, le commerçant peut aussi ouvrir Denkma avec un colis prérempli à partir des données client et de la référence de commande ; si Denkma demande une connexion, le préremplissage est conservé après l'authentification et jusqu'à l'écran de création. Les notifications E-com ouvrent directement l'onglet Commandes E-com ou la fiche CRM concernée. Les formulaires de contact déclenchent aussi des notifications et des e-mails vers le contact E-com et les groupes concernés, avec possibilité de répondre directement au client quand son e-mail est fourni. Les coordonnées client alimentent automatiquement le CRM, les clients E-com peuvent être distingués des clients physiques, et les commandes livrées sont comptées comme des ventes, pas comme des pertes." },
  ...
  },
  {
    ...
    audience: 'all',
    features: [
      { icon: 'person-outline', title: "Compte et application", description: "Réglez ici l'apparence, la langue, les préférences de compte, l'abonnement et les options liées à votre appareil. Si un compte a été créé via Google sans passer par l'inscription complète, l'application demande désormais de confirmer le pays, le téléphone et le secteur avant l'accès normal." },
      ...
      { icon: 'notifications-outline', title: "Alertes et rappels", description: "Choisissez les canaux d'alerte, les destinataires utiles, les seuils et lancez une notification de test pour vérifier le branchement push. Si Expo ou FCM sont mal configurés, le test affiche maintenant l'erreur réelle au lieu d'un faux succès. Les alertes stock très proches pour un même produit sont aussi regroupées pour réduire le bruit. Avant la création d'un compte, une installation mobile autorisée peut recevoir un rappel quotidien pour terminer l'inscription ; après la création du compte, les rappels d'activation basculent vers l'étape suivante." },
      { icon: 'card-outline', title: "Abonnement et facturation", description: "Consultez votre formule actuelle, comparez les autres offres et relisez les réglages liés au renouvellement. Les plans Starter et Pro se gèrent surtout sur mobile, tandis que le passage à Enterprise se confirme sur le web sans recréer le compte. Si la boutique indique déjà abonné, utilisez Récupérer mon abonnement : l'application resynchronise alors le plan et recharge votre statut." },
      { icon: 'checkmark-circle-outline', title: "Confirmations et retour simple", description: "Les réglages importants affichent maintenant une confirmation visuelle après sauvegarde. Depuis les CGU ou la politique de confidentialité ouvertes ici, le retour ramène directement vers Paramètres." },
      { icon: 'sync-outline', title: "Synchronisation", description: "Vérifiez l'état de connexion, les actions en attente et les modules compatibles hors ligne. Les ventes, écritures compatibles, clients, paiements clients et créations de fournisseurs sont renvoyés automatiquement dès le retour du réseau, sans rejouer plusieurs fois la même action. Un changement de boutique ne vide plus la file locale des actions déjà en attente." },
      ...
      { icon: 'lock-closed-outline', title: "Sécurité et données", description: "Le code PIN, la biométrie, l'export et la suppression du compte sont dans cette zone sensible. Après une déconnexion, la reconnexion biométrique reste disponible si elle est activée, mais l'application ne rouvre plus automatiquement le dernier compte au prochain lancement." },
