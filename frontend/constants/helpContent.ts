...
export const HELP_MODULES: HelpModule[] = [
  ...
  },
  {
    ...
    features: [
      ...
      { icon: 'storefront-outline', title: "Site e-commerce", description: "Stockman prépare un site public désactivé par défaut. L'administrateur peut l'activer, choisir la boutique liée, personnaliser le nom du site, l'accueil, les contacts, la livraison, WhatsApp, le paiement manuel et configurer le domaine. Le format recommandé pour un domaine connecté est www.votredomaine.com : créez un CNAME vers Stockman, puis redirigez votredomaine.com vers ce sous-domaine. Les réglages affichent maintenant un guide concret selon Cloudflare, OVHcloud ou GoDaddy, avec le type, le nom et la valeur DNS à saisir. Les couleurs se choisissent avec des pastilles visuelles et pilotent les boutons, badges et accents du site. La vitrine affiche une identité de boutique visible dès l'en-tête, une vraie section Contactez-nous et un formulaire public qui envoie la demande au commerçant. Le bouton E-com permet de voir le site, d'ouvrir les statistiques et d'accéder aux paramètres. Les commandes E-com arrivent dans Stockman avec notifications push, in-app et e-mail. Depuis le détail d'une commande E-com, le commerçant peut aussi ouvrir Denkma avec un colis prérempli à partir des données client et de la référence de commande. Les notifications E-com embarquent maintenant une destination directe pour ouvrir la bonne section dans l'application, comme les commandes ou le CRM. Les formulaires de contact déclenchent aussi des notifications et des e-mails vers le contact E-com et les groupes concernés, les coordonnées client alimentent automatiquement le CRM, et les commandes livrées sont comptées comme des ventes, pas comme des pertes." },
